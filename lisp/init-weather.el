...
;;copied from https://github.com/thierryvolpiatto/emacs-config/blob/main/wttr-weather.el
...
(defvar wttr-weather-default-location  "Paris" "Hangzhou")
  ...
  (let* ((inhibit-read-only t)
         ...
         (data
          (with-temp-buffer
            (call-process
             "curl" nil '(t t) nil
             "-s" (format "zh.wttr.in/%s?m" (replace-regexp-in-string " " "+" place)))
            (goto-char (point-min))
